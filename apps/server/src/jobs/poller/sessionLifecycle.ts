...
import { evaluateRulesAsync, hasTranscodeConditions, hasPauseDurationConditions } from '../../services/rules/engine.js';
...
// ============================================================================
// Pause Duration Re-evaluation
// ============================================================================

/**
 * Re-evaluate V2 rules when an existing session is paused.
 *
 * Only rules containing paused_duration_minutes conditions are evaluated.
 * This is triggered on every poll cycle where the session state is 'paused',
 * allowing rules like "notify if paused > 10 minutes" to fire once the threshold
 * is reached.
 *
 * Deduplication: checks for existing violations per (ruleId, sessionId) before inserting,
 * since the DB unique index uses ruleType which is NULL for V2 rules.
 */
export async function reEvaluateRulesOnPauseChange(
  input: PauseReEvalInput
): Promise<ViolationInsertResult[]> {
  const { existingSession, server, serverUser, activeRulesV2, activeSessions, recentSessions } =
    input;

  // Filter to only rules that have pause-duration-related conditions
  const pauseRules = activeRulesV2.filter(hasPauseDurationConditions);
  if (pauseRules.length === 0) return [];

  // Build Session object directly from the existing session row
  const session: Session = {
    id: existingSession.id,
    serverId: existingSession.serverId,
    serverUserId: existingSession.serverUserId,
    ...
    mediaTitle: existingSession.mediaTitle,
    grandparentTitle: existingSession.grandparentTitle,
    seasonNumber: existingSession.seasonNumber,
    episodeNumber: existingSession.episodeNumber,
    year: existingSession.year,
    thumbPath: existingSession.thumbPath,
    ratingKey: existingSession.ratingKey,
    startedAt: existingSession.startedAt,
    stoppedAt: null,
    durationMs: null,
    totalDurationMs: existingSession.totalDurationMs,
    progressMs: existingSession.progressMs,
    lastPausedAt: existingSession.lastPausedAt,
    pausedDurationMs: existingSession.pausedDurationMs,
    referenceId: existingSession.referenceId,
    watched: existingSession.watched,
    ...
    product: existingSession.product,
    device: existingSession.device,
    platform: existingSession.platform,
    quality: existingSession.quality,
    isTranscode: existingSession.isTranscode,
    videoDecision: existingSession.videoDecision,
    audioDecision: existingSession.audioDecision,
    bitrate: existingSession.bitrate,
    sourceVideoCodec: existingSession.sourceVideoCodec,
    sourceAudioCodec: existingSession.sourceAudioCodec,
    sourceAudioChannels: existingSession.sourceAudioChannels,
    sourceVideoWidth: existingSession.sourceVideoWidth,
    sourceVideoHeight: existingSession.sourceVideoHeight,
    sourceVideoDetails: existingSession.sourceVideoDetails,
    sourceAudioDetails: existingSession.sourceAudioDetails,
    streamVideoCodec: existingSession.streamVideoCodec,
    streamAudioCodec: existingSession.streamAudioCodec,
    streamVideoDetails: existingSession.streamVideoDetails,
    streamAudioDetails: existingSession.streamAudioDetails,
    transcodeInfo: existingSession.transcodeInfo,
    subtitleInfo: existingSession.subtitleInfo,
    channelTitle: existingSession.channelTitle,
    channelIdentifier: existingSession.channelIdentifier,
    channelThumb: existingSession.channelThumb,
    artistName: existingSession.artistName,
  ...
  // Evaluate only pause-duration-related rules
  ...
  for (const result of ruleResults) {
    ...
    // The DB unique index uses ruleType (NULL for V2), and NULL != NULL in PG,
    // so onConflictDoNothing can't catch V2 duplicates. The advisory lock
    // serializes concurrent attempts for the same session+rule pair.
    const violationResult = await db.transaction(async (tx) => {
      // Advisory lock scoped to this session+rule pair.
      await tx.execute(
        sql`SELECT pg_advisory_xact_lock(hashtext(${existingSession.id} || '::' || ${rule.id}))`
    ...
    // Execute actions (e.g., send_notification)
    // Runs outside the transaction - side effects shouldn't block violation commit.
    if (result.actions.length > 0) {
      const context: EvaluationContext = { ...baseContext, rule };
      const actionResults: ActionResult[] = await executeActions(context, result.actions);
